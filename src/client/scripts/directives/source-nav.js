angular.module('rapid-build').directive('rbaSourceNav', ['$injector', 'typeService', 'ENV',
	($injector, Type, ENV) => {
		/* COMPILE
		 **********/
		...
		const Link = (scope, iElement, iAttrs) => {
			...
			if (Type.is.function(source)) source = source();
			source = angular.copy(source); // copy to not modify constant

			if (ENV.is.dev)
				Helpers.updateDevUrls(source.code); // prep source urls for environment
